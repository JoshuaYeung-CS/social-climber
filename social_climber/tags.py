...
VALID_FLAGS = {"favorite", "want_remove", "watchlist", "deactivated", "unavailable", "random_request", "now_public", "need_archive", "archive_skip", "to_follow", "star"}
...
def all_flagged_usernames(conn: sqlite3.Connection) -> dict[str, dict]:
    """Return every tagged user with both the flag-booleans AND the
    matching `<flag>_added_at` timestamps. Callers that only need the
    booleans look up by flag name; callers that need the 'when did I
    tag this?' answer look up `<flag>_added_at`.

    The added_at fields are why the To-follow / Wait-back / Want-remove
    list rows can show 'added 3d ago' captions — without this they
    were stripped on the way through build_row()."""
    rows = conn.execute(
        "SELECT username, "
        "favorite, favorite_added_at, "
        "want_remove, want_remove_added_at, "
        "watchlist, watchlist_added_at, "
        "deactivated, deactivated_added_at, "
        "unavailable, unavailable_added_at, "
    ...
    return {
        r["username"]: {
            ...
            "favorite_added_at": r["favorite_added_at"],
            "want_remove": bool(r["want_remove"]),
            "want_remove_added_at": r["want_remove_added_at"],
            "watchlist": bool(r["watchlist"]),
            "watchlist_added_at": r["watchlist_added_at"],
            "deactivated": bool(r["deactivated"]),
            "deactivated_added_at": r["deactivated_added_at"],
            "unavailable": bool(r["unavailable"]),
            "unavailable_added_at": r["unavailable_added_at"],
            "random_request": bool(r["random_request"]),
            "random_request_added_at": r["random_request_added_at"],
            "now_public": bool(r["now_public"]),
            "now_public_added_at": r["now_public_added_at"],
            "to_follow": bool(r["to_follow"]),
            "to_follow_added_at": r["to_follow_added_at"],
            "star": bool(r["star"]),
            "star_added_at": r["star_added_at"],
        }
        for r in rows
    }
