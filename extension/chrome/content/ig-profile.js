  ...
  panel.innerHTML = `
    ...
    <div class="igt-body">
      ...
      <div class="igt-username">${escapeHtml(username)}${profile.verified ? ' <span class="igt-check" title="Verified">✓</span>' : ""}${(() => {
        // Account-type pill: business/creator. Drawn from the DOM
        // category text IG renders under the bio (e.g. "Restaurant",
        // "Public figure") — those only appear on business / creator
        // accounts, so any non-empty value tells us this isn't a
        // personal account. Useful for the "business accounts rarely
        // follow back" hypothesis.
        if (!profile.category) return "";
        const cat = String(profile.category);
        const tip = `Business account · ${cat}`;
        return ` <span class="igt-acct-pill igt-acct-business" title="${escapeHtml(tip)}">🏢 Business</span>`;
      })()}</div>
...
(async function main() {
  ...
  // and fires onLocationMaybeChanged to re-sync. 500ms cadence — two
  // regex tests per tick is cheap.
  setInterval(() => {
