#!/bin/bash
# Cron-able watchdog for the IG Tracker export bot.
#
...

{
  ...
  tail -100 "$HOME/Library/Logs/igtracker.err.log" 2>/dev/null || true
} > "$CTX_FILE"

log "Context: $CTX_FILE"

PROMPT="The IG Tracker export bot has failed $CONSEC times in a row. You are being invoked autonomously by a watchdog to diagnose. Read the diagnostic context below and write a short report.
